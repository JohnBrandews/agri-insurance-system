...
const CLOUDINARY_FOLDER = "agri-insurance-system/profile-images"
...
export async function POST(request: Request) {
  try {
    ...
    const publicId = `user-${session.id}-${timestamp}`
    const signaturePayload = `folder=${CLOUDINARY_FOLDER}&public_id=${publicId}&timestamp=${timestamp}${apiSecret}`
